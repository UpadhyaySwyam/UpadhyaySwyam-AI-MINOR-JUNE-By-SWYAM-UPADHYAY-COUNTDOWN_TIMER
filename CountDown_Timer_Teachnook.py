...
class CountDown:
    def __init__(self, root):
        ...
        self.window.title('The CountDown Timer By Swyam Upadhyay')
        self.window.configure(bg='cyan')
        #the Window length constant
        ...
        # inside this frame
        self.button_frame = Frame(self.window, bg="cyan", \
        width=240, height=40)
        self.button_frame.place(x=230, y=150)
        # It is used to show the countdown time label
        self.time_frame = Frame(self.window, bg="cyan", \
        width=480, height=120).place(x=0, y=210)

        # Tkinter Labels
        time_label = Label(self.window, text="Set Time", 
        font=("times new roman",20, "bold"), bg='cyan',fg='sea green')
        time_label.place(x=180, y=30)

        hour_label = Label(self.window, text="Hour", 
        font=("times new roman",20), bg='cyan', fg='white')
        ...
        # Tkinter Buttons
        # Reset button
        cancel_button = Button(self.window, text='Reset&Exit', 
        font=('Helvetica',12), bg="ivory2", fg="black", 
        command=self.Cancel)
        cancel_button.place(x=70, y=150)

        # Set Time Button
        # When the user will press this button the 'Start' and 'Pause' button will
        # show inside the 'self.button_frame' frame
        set_button = Button(self.window, text='Set', 
        font=('Helvetica',12), bg="ivory2", fg="black", 
        command=self.Get_Time)
        set_button.place(x=180, y=150)
    ...
    def Get_Time(self):
        ...
        try:
            ...
            # a warning message when all set to ZERO
            if s == 0 and m == 0 and h == 0:
                ...
                # Pause Button
                pause_button = Button(self.button_frame, text='Pause/Stop', 
                font=('Helvetica',12), bg="red", fg="white",
                command=self.pause_time)
                pause_button.place(x=120, y=0)
